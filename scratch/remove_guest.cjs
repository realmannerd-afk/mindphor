...
files.forEach(file => {
  ...
  content = content.replace(/const { globalMockTraces } = await import\('\.\.\/lib\/mockReviewCache'\);\n/g, '');
  ...
  // 7. General `00000` cleanup where there's an early return or specific assignment
  content = content.replace(/if \(appId === '00000000-0000-0000-0000-000000000000'\) {[\s\S]*?return new Response[\s\S]*?;}/g, '');
  content = content.replace(/if \(projectId === '00000000-0000-0000-0000-000000000000'\) {[\s\S]*?return new Response[\s\S]*?;}/g, '');
