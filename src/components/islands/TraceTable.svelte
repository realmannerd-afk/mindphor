<script lang="ts">
  ...
  async function loadData() {
    ...
    try {
      const url = `/api/feedback?app_id=${projectId}${date ? `&date=${date}` : ''}`;
  ...
  // Handle rating representation percentage
  function getScoreBadge(score: number | null) {
    if (score === null) return { class: 'text-text-muted', text: 'Pending' };
    if (score > 80) return { class: 'text-[#2D5A0E] bg-[#2D5A0E]/10 px-2 py-0.5 rounded-full', text: score.toString() + '%' };
    if (score >= 60) return { class: 'text-amber-600 bg-amber-600/10 px-2 py-0.5 rounded-full', text: score.toString() + '%' };
    return { class: 'text-[#A32D2D] bg-[#A32D2D]/10 px-2 py-0.5 rounded-full', text: score.toString() + '%' };
  }
    ...
    <div class="flex items-center gap-3">
      ...
      <div class="relative flex items-center gap-2">
        {#if statusFilter !== 'all' || scoreFilter !== 'all' || searchQuery !== ''}
          <button onclick={() => { searchQuery = ''; statusFilter = 'all'; scoreFilter = 'all'; showFilterDropdown = false; }} class="flex items-center gap-1.5 px-3 py-2.5 rounded-full border border-border-default bg-bg-surface text-text-muted hover:text-text-primary text-[12px] font-medium transition-colors focus:outline-none group" title="Clear all filters">
        ...
        {#if showFilterDropdown}
          ...
          <div class="absolute top-full right-0 mt-2 w-[220px] bg-bg-surface border border-border-default rounded-xl shadow-lg z-20 p-4">
            <div class="text-[11px] font-medium text-text-muted uppercase mb-2 tracking-wider">Source</div>
            ...
            <div class="text-[11px] font-medium text-text-muted uppercase mb-2 tracking-wider">Rating</div>
    ...
    <div class={`flex flex-col overflow-y-auto ${overviewLink ? 'max-h-[398px]' : 'flex-1 min-h-0'}`}>
      ...
      {:else if traces.length === 0}
        <div class="py-16 text-center flex flex-col items-center justify-center">
          ...
          <p class="text-[13px] text-text-muted max-w-[300px] mb-5 leading-relaxed">Integrate your feedback widget or review sync SDK to start seeing feedback logs here.</p>
        </div>
      {:else if filteredTraces.length === 0}
        <div class="py-16 text-center flex flex-col items-center justify-center">
          ...
          <button 
            onclick={() => { searchQuery = ''; statusFilter = 'all'; scoreFilter = 'all'; }} 
      ...
      {:else}
        {#each filteredTraces as trace, index}
          ...
          <div class={`grid grid-cols-12 gap-4 px-6 py-4 items-center text-[13px] hover:bg-bg-subtle/40 transition-colors group ${index !== filteredTraces.length - 1 ? 'border-b border-border-faint' : ''}`}>
            
            <div class="col-span-1 text-text-muted font-mono text-[12px]">{index + 1}</div>
            
            <div class="col-span-6 text-text-primary font-medium truncate pr-4" title={trace.input}>{trace.input}</div>
            
            <div class="col-span-1 flex items-center justify-center">
              <Logo domain={trace.model} title={trace.model.replace('_', ' ')} alt={trace.model} className="w-5 h-5 object-contain" />
            </div>
