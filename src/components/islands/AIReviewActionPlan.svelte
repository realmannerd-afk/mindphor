<script lang="ts">
  ...
  onMount(async () => {
    try {
      const res = await fetch(`/api/review-action-plan?feedback_id=${feedbackId}`);
      const data = await res.json();
      
      if (!res.ok) throw new Error(data.error || 'Failed to generate action plan');
...
<div class={`bg-bg-surface border border-border-default rounded-[16px] p-6 flex flex-col relative overflow-hidden ${className}`}>
  <div class="flex items-center justify-between mb-4 relative z-10">
    <div class="flex items-center gap-2.5">
      ...
      <h3 class="text-[14px] font-bold text-text-primary tracking-wide">Review Action Plan</h3>
    </div>
  ...
  {:else}
    <div class="text-[13px] text-text-muted italic py-4">No recommendations available for this review.</div>
  {/if}
</div>
